...
<div>
    ...
    <div class="p-6 mt-5 bg-white border-2 border-green-200 rounded-lg shadow-lg md:mx-10">
        <div class="grid grid-cols-2 md:space-x-10">
            <div class="col-span-2 md:col-span-1">
                <label for="" class="label">Select Brand</label>
                <select id=" category_id" name="category_id" wire:model="produk"
                    class="block w-1/2 px-2 py-2 mt-1 text-xs border border-gray-300 rounded shadow focus:border-green-200 focus:outline-none focus:ring-2 focus:ring-green-300">
                    <option value="" selected>Brand ... </option>
                    @if ($company)
                        @foreach ($company as $item)
                            @foreach ($item->plant as $plant)
                                @foreach ($plant->brand as $brand)
        ...
        @if ($document)
            <div class="mt-6 overflow-hidden overflow-x-auto border border-gray-200 shadow-md sm:rounded">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="text-sm bg-white divide-y divide-gray-200" x-data="{ selected:null }">
                        @foreach ($document->document as $doc)
                            <tr>
                                ...
                                @if ($doc->pivot->nama_dokumen == null)
                                    ...
                                    <td>
                                        <button type="button" wire:loading.attr="disabled"
                                            ...
                                            wire:target="uploadDokumen({{ $doc->id }}, '{{ Str::substr($doc->nama_dokumen, 0, 150) }}'), nama_dokumen"
                                            ...
                                            wire:click="uploadDokumen({{ $doc->id }}, '{{ Str::substr($doc->nama_dokumen, 0, 150) }}')">
                                ...
                                @else
                                    ...
                                    <td class="px-6 py-4 font-semibold gap-4">
                                        @if ($doc->pivot->status == 1)
                                            <button
                                                wire:click="$emit('openModal', 'sertifikasi.modal.edit-dokumen', {{ json_encode(['id' => $doc->id, 'produk' => $produk]) }})"
                                                class="px-2 py-1 mx-1 mb-1 text-sm text-white bg-indigo-500 rounded-md hover:bg-indigo-600">Edit</button>
                                            <a href="{{ asset('storage/checklist-dokumen/' . $document->plant->perusahaan->nama_perusahaan . '/' . $doc->pivot->nama_dokumen) }}"
                                                target="_blank"
                                                class="px-2 py-1 mx-1 text-sm text-white bg-green-500 rounded-md hover:bg-green-600">
                                                Preview
                                            </a>
                                            {{-- <button
                                                class="px-2 py-1 mx-1 text-sm text-white bg-green-500 rounded-md hover:bg-green-600">Preview</button> --}}
                                        @endif
                                    </td>
                                @endif
                                <td class="px-6 py-4 font-semibold text-center">
                                    @switch($doc->pivot->status)
                                        @case(0)
                                            <p class="px-2 py-0 mx-1 text-xs text-white bg-red-500 rounded-lg">
                                                kosong</p>
                                        @break
                                        @case(1)
                                            <p class="px-2 py-0 mx-1 text-xs text-white bg-yellow-500 rounded-lg">
                                                Belum
                                                diapprove</p>
                                        @break
                                        @case(2)
                                            <p class="px-2 py-0 mx-1 text-xs text-white bg-green-500 rounded-lg">
                                                Approved</p>
                                        @break
                                        @default
                                    @endswitch
