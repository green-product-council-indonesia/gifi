            ...
            <tbody class="text-sm bg-white divide-y divide-gray-200">
                @forelse ($data as $item)
                    <tr wire:loading.remove wire:target="previousPage, nextPage, gotoPage">
                        ...
                        <td class="px-6 py-2">
                            <span
                                class="px-2 py-0 text-xs text-white bg-blue-500 rounded-lg">{{ $item->kategoriSertifikasi->nama_kategori }}</span>
                        </td>
                        <td class="px-6 py-2 text-xs font-semibold whitespace-nowrap">
                            {{ Carbon\Carbon::parse($item->tgl_pendaftaran)->locale('id')->isoFormat('MMMM Y') }}
                        </td>
                        <td class="flex justify-center px-2 py-2 text-xs font-semibold whitespace-nowrap">
                            <a href="/approve/sertifikasi/{{ $item->id }}/{{ $item->slug }}"
