<div>
        ...
        <div class="grid grid-cols-12 gap-4 p-8">
            <div class="col-span-12 space-y-4 md:col-span-5">
                <div class="flex flex-col">
                    <label class="mb-2 text-xs font-semibold">Select Kategori Dokumen</label>
                    <select wire:model="kategori_dokumen"
                        ...
                        <option value="">Select Kategori Dokumen</option>
                        @foreach ($dokumen as $data)
                            <option value="{{ $data->id }}">{{ $data->nama_kategori_dokumen }}</option>
                        @endforeach
                    </select>
            ...
            <div class="col-span-12 space-y-4 md:col-span-5">
                <div class="flex flex-col">
                    <label class="mb-2 text-xs font-semibold ">Select Kategori Sertifikasi</label>
                    <select wire:model="kategori_sertifikasi"
                        class="w-full px-5 text-xs bg-white border border-gray-300 rounded-md shadow-sm appearance-none focus:outline-none focus:ring-green-400 focus:border-green-400 ">
                    ...
                    @error('kategori_sertifikasi')
                        <span class="error">{{ $message }}</span>
                    @enderror
                </div>
            </div>
            <div class="col-span-12 space-y-4 md:col-span-2">
                <div class="flex flex-col">
                    <label class="mb-2 text-xs font-semibold ">Total Bobot</label>
                    <input type="text" placeholder="Bobot" class="form-input" wire:model="total_bobot">
                    @error('total_bobot')
                        <span class="error">{{ $message }}</span>
                    @enderror
                </div>
    ...
    <div class="flex justify-end px-4 py-3 space-x-4 bg-gray-50">
        <button wire:click.prevent="tambahBobot"
